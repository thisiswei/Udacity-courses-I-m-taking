...
def bowling(balls):
    ...
    total_s, frame, pos = 0, 1, 0
    while frame <= 9:
        current_s  = balls[pos]
        next_s     = balls[pos+1]
        next_two_s = balls[pos+2]
        if current_s == 10 :
            total_s += current_s + next_s + next_two_s
            pos += 1
        if current_s < 10:
            pos += 2
            if  current_s + next_s == 10 :
                total_s += current_s + next_s + next_two_s
            else:
                total_s += current_s + next_s
        frame +=1 
        
    return total_s + sum( balls[pos:] )
...
def test_bowling():
    ...
    print 'oh la la dec 08 2012 .. keep on, wei! keep on'
