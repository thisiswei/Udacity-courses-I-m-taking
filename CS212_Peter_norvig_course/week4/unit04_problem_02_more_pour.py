...
def more_pour_problem(capacities, goal, start=None):
    ...
    def more_successor(state):
        indices = range(len(state))
        s = {}
        for i in indices:
            s[replace(state,i,capacities[i])] = ('fill', i)
            s[replace(state,i,0)]             = ('empty', i)
            for j in indices:
                if i != j:
                    amount = min(state[i], capacities[j] - state[j])
                    state2 = replace(state, i, state[i] - amount)
                    s[replace(state2, j, state[j] + amount)] = ('pour', i, j)
        return s 
    if start is None: start=(0,)*len(capacities) 
    return shortest_path_search(start,more_successor,is_goal)
